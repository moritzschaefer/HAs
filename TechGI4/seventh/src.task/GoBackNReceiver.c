    ...
    while (1) {
        ...
        if(data->hasErrors) {
            ...
            if(data->seqNo == lastReceivedSeqNo+1) {
                lastReceivedSeqNo += 1;
                if(data->size ==sizeof(*data)) { //last package
                    printf("Total transmission bytes: %d, total good bytes: %d\n", totalBytes, goodBytes);
                    fclose(output);
                } else {
         ...
         * - Check if packet is x
         *   # the one we are expecting x
         *   # an older one x
         *   # too new (we missed at least one packet) x
         ...
         * - Update goodBytes x
         * - If this was the last packet of the transmission x
         *   (data->size == sizeof(*data)) close the file with fclose and x
         *   output totalBytes and goodBytes x
         *
